...
作成済みビルド: `output/PixelStrict-windows-x64.zip`（展開してpixelstrict.exeを起動）、`output/PixelStrict-android-arm64-debug.apk`（検証用）。[検証結果](VALIDATION.md)と[比較画像](samples/comparison.png)も参照してください。Windows配布時はexe単体ではなくZIP内のDLL・dataを含むフォルダー全体が必要です。
...
2. Grid: Auto、またはManualで**出力幅（セル数）**を指定。高さは縦横比から整数に丸めます。
3. Colors: Auto / 16 / 24 / 32。指定数は透明色を含む上限で、不要な色は追加しません。
4. Surface smoothing / Edge protectionを弱・中・強から選択。MedianはOff / 半径1px。
5. 変換後、Before / Afterを比較してPNGを保存。片方をズーム／パンすると、もう片方も同じ倍率・位置に同期します。変換・再変換時は表示位置を保持し、新しい画像を読み込むと全体表示に戻ります。補間はNearest Neighborのみ。

保存PNGは**論理ピクセル原寸**です。例えば192×192の6pxセルを選ぶと32×32を保存します。元の大きさに拡大した画像ではありません。入力alphaは128を閾値に0 / 255へ二値化し、透明画素はRGBA=(0,0,0,0)に統一します。入力上限は64 MiB、各辺8192px、合計16,777,216画素。出力上限は262,144セルです。
...
4. 元画像から整数セル候補を生成。1254px幅では627 / 418 / 209 / 114などを含みます。非整除寸法にも候補を作成し、入力範囲を整数区間へ分割。全入力画素を1回ずつ所属させます。
...
10. 新規RGBAバッファへ1セル1画素で描画しPNG encode。アンチエイリアス、半透明境界、サブピクセル座標は発生しません。
...
```powershell
cargo run --release --manifest-path core/Cargo.toml --bin pixelstrict -- samples/house-pseudo.png output.png --grid auto --colors 24 --smoothing 3 --edge 2 --report report.json
# 手動の出力幅
cargo run --release --manifest-path core/Cargo.toml --bin pixelstrict -- input.png strict.png --grid 64 --colors 16 --median
```

...
`shaded-stroke`では、線の内側で細い陰影を過剰に保護して元の平均より暗い色を選ぶ問題を検証します。v0.1.6+7では、安定した直線部分を元画像の明るさへ近づけ、途中の本来の明暗変化・線幅・端点の位置を保持。縦横、両出力モード、単色矩形と決定論性に加え、横棒と2セルの色付き小形状を消さないテストを追加しました。5セルの連続性がない端点付近や明暗の切り替わり付近は補助対象外で、色ムラが残る場合があります。

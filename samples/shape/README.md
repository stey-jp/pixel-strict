...
`shaded-stroke`では、線の内側で細い陰影を過剰に保護して元の平均より暗い色を選ぶ問題を検証します。v0.1.8+9では境界位置を保護した2行も含め、線全体の明度を検証。v0.1.7+8で明度がずれた境界4セルも整え、線幅・端点・明暗の変化位置を維持します。縦横、両出力モード、単色矩形と決定論性を確認しています。

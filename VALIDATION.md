...
## v0.1.9+10 — 斜めの帯と細線の連続性（2026-09-24）

斜めの面の陰影を個別のLINEとして過剰に評価する問題に対し、Rustの`decide`へ元画像の斜め方向の明度照合を追加しました。浅い斜線・急な斜線・両傾斜を有限の方向候補で調べ、連続する4区間以上の支持を要求。1区間ずつ調べ、切れ目でその側の探索を止めます。整数座標の±1pxで階段の位相差を許容し、補間は行いません。追加バッファ・依存なし。境界位置が固定済みのセル、小形状、透明画素を含むセルは従来の保護を優先します。

- 通常の面は元セルの平均明度で候補色を評価。両側が同じ背景の細線は、背景との平均で線色を薄めず、連続性を確認した線の明暗を選びます。色相差の大きい候補を除き、明度幅20/255未満の微小な面ノイズには適用しません。中間色に近い曖昧な面の選択も従来のままです。
- `diagonal-shades`を追加。1:2の斜めの帯で、明暗切替の両側の元セル平均に対する明度MAEは旧版 **16.894 / 16.837 → 0.728 / 1.060**。上下反転・90度回転・Preserve / Logical・全セル単色・PNG決定論性を検証しました。ManualはGrid 48・P=3、AutoはGrid 72・P=2です。
- 1:2 / 2:5 / 1:1の2px細線、元からある段差と9pxの切れ目を追加検証。旧版で全消失した低コントラストの1:2線は、切れ目を除く33列すべてで保持。透明な切れ目と2セルの色付き小形状も保持します。元線がセルより細い場合、出力線幅は整数セル単位に制約されます。
- 実画像`original.png`を旧版と同条件（Auto / Colors 32 / Smoothing 2 / Edge 2 / Shape 2 / Median Off）で比較。**Grid 418×418・P=3、1254×1254・32色**は不変。2,883セル（25,947画素）が変化し、alphaは全画素不変です。変更セルの元セル平均に対する明度MAEは **14.506→5.194**、RGB MAEは **13.581→5.565**。
- 添付に対応する右側の斜めの赤茶色の帯2本（2,823セル）では、明度MAE **3.941→3.621**、誤差12/255超のセル **138→89**。上部の細い帯（582セル）は明度MAE **8.141→7.852**、誤差12超 **93→83**。領域はセル中心`x=3gx+1, y=3gy+1`、`267≤gx<340`、`950<y+0.42x<1010`または`1128<y+0.42x<1184`、上部は`808<y+0.42x<832`で固定しています。これは局所比較で、セル平均誤差は輪郭品質そのものではありません。元画像・派生比較はローカルのみに保存しています。
- 既存houseのAuto / Surface弱 / Surface強はPNGバイト単位で不変、参照RGB MAE **2.819 / 2.654 / 2.663**。既存Shape fixtureは`blended-frame`・`interrupted-stroke`のManualと`shaded-stroke`のAutoのみPNGが変化し、既存の選択Gridは維持。対応する画像・候補評価JSONを更新しました。
- `cargo test --release --locked --manifest-path core/Cargo.toml`: **45件成功**（3件追加）。外形、縦線、窓枠、単独ノイズ、小形状、Auto、非整除Logical、Preserve、決定論性を継続確認。`cargo clippy --release --locked --manifest-path core/Cargo.toml --all-targets -- -D warnings`、`cargo fmt --manifest-path core/Cargo.toml --check`: **成功、指摘なし**。
- `flutter analyze --no-pub`、widgetテスト、Windows FFI統合テスト、Windows Releaseビルド: **成功**。固定ボタン・版表示・同期preview・保存・両出力モード・設定変更時の保存無効化を維持。exeと同梱pubspecは**0.1.9+10**です。
- 配布用DLLで実画像と11種のManual fixtureが検証済みPNGと一致。実画像のPreserveはLogicalを3×3単色矩形にしたものと全画素一致し、再変換のPNGも一致。旧／新DLLをwarm-up後、順序を交互に各5回計測した中央値は **421.14ms→487.24ms（約15.7%増）**。方向照合の追加コストで、ローカル負荷による変動を含みます。

斜めの塗りと線の色選択は改善していますが、上部の細い帯や交差部には乱れが残ります。明度差が小さい線、複雑な接触部分、1セル未満の形は今回の近似では完全には復元できません。次は斜線の階段幅と境界位置を評価する補正が候補です。Android / macOS / iOSは今回再ビルド・実機検証していません。Web成果物・監査対象URLなし。以下は過去版の記録です。
